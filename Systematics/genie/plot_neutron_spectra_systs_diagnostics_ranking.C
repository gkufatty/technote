...
//
// Reads make_neutron_spectra_systs_diagnostics.root (produced by
// make_neutron_spectra_systs_diagnostics.C) and answers the actual
// question behind that script: for each of the technote's five physics
// categories (FSI, MEC, RES, QE, DIS) and its recommended diagnostic
// variable(s), does shifting the category's systematics by +-1 sigma
// actually move that diagnostic -- i.e. is the technote's priority ranking
// (Table 1) and diagnostic choice (Table 2) borne out by the MC, or not?
//
...
void plot_neutron_spectra_systs_diagnostics_ranking()
{
  ...
  // NeutronMultSystDiagnostics.cxx's kFinalStateTopologyBinVar for the
  // exact nP/nN/nPi definition of each bin (mirrors cuts.C's kFSI_*).
